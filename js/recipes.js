...
// Normalisering af rating/reviews (så "votes"/"reviews"/"ratingCount" m.v. bliver ens)
function normalizeRecipe(r) {
  const ratingRaw =
    r.rating ?? r.stars ?? r.score ?? r.avgRating ?? null;
  const rating = (typeof ratingRaw === 'number')
    ? ratingRaw
    : (ratingRaw ? parseFloat(String(ratingRaw).replace(',', '.')) : null);

  const votesRaw =
    r.votes ?? r.reviews ?? r.reviewCount ?? r.ratingCount ?? r.votesCount ??
    (Array.isArray(r.ratings) ? r.ratings.length : null);
  const votes = (typeof votesRaw === 'number')
    ? votesRaw
    : (votesRaw ? parseInt(votesRaw, 10) : 0);

  return {
    ...r,
    rating: (rating != null && !Number.isNaN(rating)) ? rating : 4,
  ...
  return `
    <a href="/pages/opskrift?slug=${encodeURIComponent(slug)}"
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="text-sm mt-1">${starHtml} <span class="text-stone-500">(${rec.votes})</span></div>
      </div>
    </a>
  `;
}
