...
async function safeMountPRWidget(recipe){
  try {
    ...
    // vælg key fra mapping (støt både chooseWidgetKeys og chooseWidgetKeyFrom)
    let key = null;
    ...
    if (!document.querySelector(slotSel)) {
      const mainCol = document.querySelector('#recipeMainCol') || document.querySelector('.md\\:col-span-2');
      if (mainCol) {
...
function updateFavUI(btn, isFav){
  ...
  if (isFav){ btn.classList.add('bg-rose-50','border-rose-300'); svg?.classList.remove('opacity-30'); if (label) label.textContent = 'Gemt'; }
  else { btn.classList.remove('bg-rose-50','border-rose-300'); svg?.classList.add('opacity-30'); if (label) label.textContent = 'Gem'; }
}
...
async function init(){
  ...
  catch (e){ titleEl.textContent = 'Kunne ikke indlæse opskrifter'; console.error(e); return; }

  const recipe = findBySlug(data, slug);
  if (!recipe){ titleEl.textContent = 'Opskrift ikke fundet'; return; }
  ...
  userRating.addEventListener('click', (e)=>{
    ...
    const u = currentUser(); if (!u){ showToast('Du skal være logget ind for at bedømme'); return; }
    const n = +b.getAttribute('data-star');
    [...userRating.querySelectorAll('svg')].forEach((svg, idx)=>{ svg.classList.toggle('opacity-50', idx >= n); });
  ...
  favBtn.addEventListener('click', (e)=>{
    e.preventDefault();
    const u = currentUser(); if (!u){ showToast('Du skal være logget ind for at gemme'); return; }
    const ok = toggleFavorite(recipe.slug);
    window.dispatchEvent(new CustomEvent('favorite:toggled', { detail: { slug: recipe.slug, ok, ts: Date.now() }}));
  ...
  // PriceRunner (nu i main-kolonnen)
